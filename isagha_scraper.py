...
CURRENCY_URL = "https://market.isagha.com/prices/currencies"
...
def get_prices():
    ...
    # صفحة الدولار منفصلة تمامًا عن صفحة الذهب في الموقع
    currency_resp = requests.get(CURRENCY_URL, headers=HEADERS, timeout=15)
    currency_resp.raise_for_status()
    currency_soup = BeautifulSoup(currency_resp.text, "html.parser")

    # جدول الذهب فقط - الفضة والعملات التانية مش هنقرب منها خالص
    gold_table = _parse_table(soup, "أسعار الذهب في مصر")
    currency_table = _parse_table(currency_soup, "أسعار العملات مقابل الجنيه")
