...
from calendar_scraper import get_calendar, save_manual_override
...
CALENDAR_CACHE_SECONDS = 1800  # 30 دقيقة - قللنا عدد الطلبات عشان نتجنب حظر المصدر (429)
...
# ===== ذاكرة تاريخية لبناء شموع حقيقية بمرور الوقت (مخزنة خارجيًا بشكل دائم) =====
...
@app.route("/api/set-actual", methods=["POST"])
def set_actual():
    ...
    if not admin_secret or secret != admin_secret:
        return jsonify({"error": "كلمة السر غير صحيحة"}), 403

    date_cairo = data.get("date_cairo")
    title_keyword = data.get("title_keyword")

    if not (date_cairo and title_keyword):
        return jsonify({"error": "محتاج date_cairo و title_keyword"}), 400

    fields = {
        "actual": data.get("actual"),
        "forecast": data.get("forecast"),
        "previous": data.get("previous"),
        "impact_text": data.get("impact_text"),
        "impact_color": data.get("impact_color"),
    }

    try:
        save_manual_override(date_cairo, title_keyword, fields)
        _calendar_cache["timestamp"] = 0  # إجبار تحديث فوري في المرة الجاية
...
@app.route("/api/bls-debug")
def bls_debug():
    from bls_scraper import get_bls_actuals
    try:
        return jsonify(get_bls_actuals())
    except Exception as e:
        return jsonify({"error": str(e)}), 502
