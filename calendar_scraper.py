...
# مواعيد رسمية معلنة مسبقًا (BLS + الفيدرالي) لحد 3 شهور قدام، عشان الصفحة
# تعرض التواريخ القادمة حتى قبل ما تدخل "أسبوع" فيد ForexFactory.
# "المتوقع" مش موجود هنا عمدًا (مستحيل يتحدد قبل الموعد بشهور)، لكن "السابق"
# بيتسجل من BLS. المتوقع هيتسجل لوحده تلقائيًا أول ما يظهر في فيد الأسبوع.
KNOWN_US_EVENTS = [
    # (الاسم, تاريخ ووقت أمريكا الشرقية, نوع الحدث, الأهمية)
    ("مؤشر أسعار المستهلكين (CPI m/m)", datetime(2026, 10, 14, 8, 30), "cpi_mm", "High"),
    ("تقرير الوظائف الأمريكي (Non-Farm)", datetime(2026, 10, 2, 8, 30), "nfp", "High"),
    ("معدل البطالة الأمريكي (Unemployment Rate)", datetime(2026, 10, 2, 8, 30), "unemployment_rate", "High"),
    ("مؤشر أسعار المنتجين (PPI m/m)", datetime(2026, 10, 15, 8, 30), "ppi_mm", "Medium"),
    ("قرار الفائدة الأمريكية (FOMC)", datetime(2026, 10, 28, 14, 0), None, "High"),

    ("مؤشر أسعار المستهلكين (CPI m/m)", datetime(2026, 11, 10, 8, 30), "cpi_mm", "High"),
    ("تقرير الوظائف الأمريكي (Non-Farm)", datetime(2026, 11, 6, 8, 30), "nfp", "High"),
    ("معدل البطالة الأمريكي (Unemployment Rate)", datetime(2026, 11, 6, 8, 30), "unemployment_rate", "High"),
    ("مؤشر أسعار المنتجين (PPI m/m)", datetime(2026, 11, 13, 8, 30), "ppi_mm", "Medium"),

    ("مؤشر أسعار المستهلكين (CPI m/m)", datetime(2026, 12, 10, 8, 30), "cpi_mm", "High"),
    ("تقرير الوظائف الأمريكي (Non-Farm)", datetime(2026, 12, 4, 8, 30), "nfp", "High"),
    ("معدل البطالة الأمريكي (Unemployment Rate)", datetime(2026, 12, 4, 8, 30), "unemployment_rate", "High"),
    ("مؤشر أسعار المنتجين (PPI m/m)", datetime(2026, 12, 15, 8, 30), "ppi_mm", "Medium"),
    ("قرار الفائدة الأمريكية (FOMC)", datetime(2026, 12, 9, 14, 0), None, "High"),
]
...
def get_calendar(min_impact="Low", countries=("USD",)):
    ...
    # ===== تسجيل مسبق للمواعيد المعروفة لحد 3 شهور قدام (بالسابق بس، من BLS) =====
    seed_archive = calendar_archive.load_archive()
    for name, dt_naive, bls_key, impact in KNOWN_US_EVENTS:
        dt_ny = dt_naive.replace(tzinfo=NY_TZ)
        if dt_ny.astimezone(timezone.utc) < now:
            continue  # حدث فات، مش محتاجين نزرعه
        dt_cairo_seed = dt_ny.astimezone(CAIRO_TZ)
        seed_key = f"{dt_cairo_seed.strftime('%Y-%m-%d')}|{name}"
        existing_seed = seed_archive.get(seed_key, {})
        previous_value = bls_actuals.get(bls_key) if bls_key else None
        seed_archive[seed_key] = {
            "title": name,
            "date_cairo": dt_cairo_seed.strftime("%Y-%m-%d"),
            "time_cairo": dt_cairo_seed.strftime("%H:%M"),
            "weekday_ar": ["الإثنين", "الثلاثاء", "الأربعاء", "الخميس", "الجمعة", "السبت", "الأحد"][dt_cairo_seed.weekday()],
            "country": "USD",
            "impact": impact,
            "impact_ar": _impact_arabic(impact),
            "previous": existing_seed.get("previous") or previous_value,
            "forecast": existing_seed.get("forecast"),  # هيتسجل لوحده تلقائيًا لما يظهر في فيد الأسبوع
            "actual": existing_seed.get("actual"),
            "explanation": existing_seed.get("explanation") or _explain(name),
            "scenarios": existing_seed.get("scenarios") or _scenario_breakdown(_event_category(name) or "inverse"),
            "manual_impact_text": existing_seed.get("manual_impact_text"),
            "manual_impact_color": existing_seed.get("manual_impact_color"),
            "actual_is_manual": existing_seed.get("actual_is_manual", False),
        }
    calendar_archive.save_archive(seed_archive)
    ...
    # ===== الأرشيف الدائم: نسجل كل حدث شفناه عشان يفضل متاح حتى لو
    # اختفى من "الأسبوع الحالي" عند المصدر =====
    archive_fields = ("title", "date_cairo", "time_cairo", "weekday_ar", "country",
                       "impact", "impact_ar", "previous", "forecast", "actual",
                       "explanation", "scenarios", "manual_impact_text",
                       "manual_impact_color", "actual_is_manual")
    ...
    to_archive = [
        {f: ev.get(f) for f in archive_fields}
        for ev in (events + past_events) if _has_any_data(ev)
    ]
    archive = calendar_archive.load_archive()
    archive = calendar_archive.upsert_events(archive, to_archive)
    archive = calendar_archive.prune_old_entries(archive)
    calendar_archive.save_archive(archive)

    # دلوقتي نبني القايمة النهائية من الأرشيف نفسه (مش بس من نتيجة الجلب الحالية)
    # عشان أي حدث اتسجل قبل كده يفضل ظاهر حتى لو مصدر الأسبوع اتغيّر
    unique_events = []
    unique_past_events = []

    for entry in archive.values():
        try:
            dt_naive = datetime.strptime(f"{entry['date_cairo']} {entry['time_cairo']}", "%Y-%m-%d %H:%M")
        except (KeyError, ValueError):
            continue
        dt_cairo_entry = dt_naive.replace(tzinfo=CAIRO_TZ)
        entry_is_future = dt_cairo_entry.astimezone(timezone.utc) >= now

        entry_forecast = _parse_number(entry.get("forecast"))
        entry_previous = _parse_number(entry.get("previous"))
        entry_actual = _parse_number(entry.get("actual"))
        entry_trend = None
        if entry_forecast is not None and entry_previous is not None:
            if entry_forecast > entry_previous:
                entry_trend = "up"
            elif entry_forecast < entry_previous:
                entry_trend = "down"
            else:
                entry_trend = "flat"

        entry_category = _event_category(entry.get("title", ""))
        final_event = dict(entry)
        final_event["trend_forecast_vs_previous"] = entry_trend

        if entry_is_future:
            sentiment = _gold_sentiment(entry_category, entry_trend)
            final_event["expected_gold_impact"] = sentiment[0] if sentiment else None
            if _has_any_data(final_event):
                unique_events.append(final_event)
        else:
            outcome_note = None
            actual_vs_forecast = None
            if entry_actual is not None and entry_forecast is not None:
                if entry_actual > entry_forecast:
                    outcome_note = "جاءت النتيجة الفعلية أعلى من المتوقع"
                    actual_vs_forecast = "higher"
                elif entry_actual < entry_forecast:
                    outcome_note = "جاءت النتيجة الفعلية أقل من المتوقع"
                    actual_vs_forecast = "lower"
                else:
                    outcome_note = "جاءت النتيجة الفعلية مطابقة للمتوقع"
                    actual_vs_forecast = "same"
            final_event["outcome_note"] = outcome_note
            final_event["actual_vs_forecast"] = actual_vs_forecast
            if _has_any_data(final_event):
                unique_past_events.append(final_event)
