...
def prune_old_entries(archive_dict, keep_days_past=30, keep_days_future=90):
    """بيشيل الأحداث القديمة جدًا عشان الأرشيف مايكبرش من غير حد"""
    from datetime import datetime, timedelta
    today = datetime.now()
    cutoff_past = (today - timedelta(days=keep_days_past)).strftime("%Y-%m-%d")
    cutoff_future = (today + timedelta(days=keep_days_future)).strftime("%Y-%m-%d")
    return {
        k: v for k, v in archive_dict.items()
        if cutoff_past <= v.get("date_cairo", "9999-99-99") <= cutoff_future
    }
